...
model_exec(LogisticRegression, pos_x_train, pos_x_test, pos_y_train, pos_y_test, pos_start, int(np.ceil(np.linspace(pos_start, pos_x_train.shape[1], skip)[2] - np.linspace(pos_start, pos_x_train.shape[1], skip)[1])), 'pos',
           pos_s, 'chi2', C=0.1, solver='lbfgs', multi_class='ovr')
model_exec(LogisticRegression, neg_x_train, neg_x_test, neg_y_train, neg_y_test, neg_start, int(np.ceil(np.linspace(neg_start, neg_x_train.shape[1], skip)[2] - np.linspace(neg_start, neg_x_train.shape[1], skip)[1])), 'neg',
           neg_s, 'chi2', C=0.1, solver='lbfgs', multi_class='ovr')
model_exec(LogisticRegression, unc_x_train, unc_x_test, unc_y_train, unc_y_test, unc_start, int(np.ceil(np.linspace(unc_start, unc_x_train.shape[1], skip)[2] - np.linspace(unc_start, unc_x_train.shape[1], skip)[1])), 'unc',
           unc_s, 'chi2', C=0.1, solver='lbfgs', multi_class='ovr')
model_exec(LogisticRegression, bag_x_train, bag_x_test, bag_y_train, bag_y_test, bag_start, int(np.ceil(np.linspace(bag_start, bag_x_train.shape[1], skip)[2] - np.linspace(bag_start, bag_x_train.shape[1], skip)[1])), 'bag',
           bag_s, 'chi2', C=0.1, solver='lbfgs', multi_class='ovr')
model_exec(LogisticRegression, wnt_x_train, wnt_x_test, wnt_y_train, wnt_y_test, wnt_start, int(np.ceil(np.linspace(wnt_start, wnt_x_train.shape[1], skip)[2] - np.linspace(wnt_start, wnt_x_train.shape[1], skip)[1])), 'wnt',
           wnt_s, 'chi2', C=0.1, solver='lbfgs', multi_class='ovr')
model_exec(LogisticRegression, thl_x_train, thl_x_test, thl_y_train, thl_y_test, thl_start, int(np.ceil(np.linspace(thl_start, thl_x_train.shape[1], skip)[2] - np.linspace(thl_start, thl_x_train.shape[1], skip)[1])), 'thl',
           thl_s, 'chi2', C=0.1, solver='lbfgs', multi_class='ovr')
model_exec(LogisticRegression, tag_x_train, tag_x_test, tag_y_train, tag_y_test, tag_start, int(np.ceil(np.linspace(tag_start, tag_x_train.shape[1], skip)[2] - np.linspace(tag_start, tag_x_train.shape[1], skip)[1])), 'tag',
           tag_s, 'chi2', C=0.1, solver='lbfgs', multi_class='ovr')

# MIC
model_exec(LogisticRegression, lit_x_train, lit_x_test, lit_y_train, lit_y_test, lit_start, int(np.ceil(np.linspace(lit_start, lit_x_train.shape[1], skip)[2] - np.linspace(lit_start, lit_x_train.shape[1], skip)[1])), 'lit',
...
pos_sel_df = metric_data_prep(x_pos_df, pos_s, n=31, flag='chi2')
neg_sel_df = metric_data_prep(x_neg_df, neg_s, n=35, flag='chi2')
unc_sel_df = metric_data_prep(x_unc_df, unc_s, n=8, flag='chi2')
bag_sel_df = metric_data_prep(x_bag_df, bag_s, n=734, flag='chi2')
wnt_sel_df = metric_data_prep(x_wnt_df, wnt_s, n=27, flag='chi2')
thl_sel_df = metric_data_prep(x_thl_df, thl_s, n=2, flag='chi2')
tag_sel_df = metric_data_prep(x_tag_df, tag_s, n=208, flag='chi2')
...
model_exec(LogisticRegression, chi_x_train, chi_x_test, chi_y_train, chi_y_test, 10, 115, 'chi',
...
chi_sel_df = metric_data_prep(df_chi_eng, chi_s, n=240, flag='chi2')

x = model_eval(LogisticRegression, chi_sel_df, y_relavence_lit_df, title='chi2', C=0.1, solver='lbfgs', multi_class='ovr')
x.to_csv('log_strategy_chi.csv')
# MIC 
# for chi2 data after feateure engineering
lit_mic_sel_df = metric_data_prep(x_lit_df, lit_s, n=6, flag='mic')
pos_mic_sel_df = metric_data_prep(x_pos_df, pos_s, n=31, flag='mic')
neg_mic_sel_df = metric_data_prep(x_neg_df, neg_s, n=67, flag='mic')
unc_mic_sel_df = metric_data_prep(x_unc_df, unc_s, n=8, flag='mic')
...
thl_mic_sel_df = metric_data_prep(x_thl_df, thl_s, n=2, flag='mic')
tag_mic_sel_df = metric_data_prep(x_tag_df, tag_s, n=292, flag='mic')
...
model_exec(LogisticRegression, mic_x_train, mic_x_test, mic_y_train, mic_y_test, 10, 131, 'mic',
...
mic_sel_df = metric_data_prep(df_mic_eng, mic_s, n=1189, flag='mic') #best result

model_eval(LogisticRegression, mic_sel_df, y_relavence_lit_df,
           title='chi2', C=0.1, solver='lbfgs', multi_class='ovr')
