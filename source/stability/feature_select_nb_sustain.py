...
mic_s.to_csv("nb_sustain_mic_feat.csv")
...
x = model_eval(GaussianNB, mic_sel_df, y_relavence_lit_df,
           title='mic')
x.to_csv('nb_sustain_mic.csv')

x_sel_df = pd.concat([x_lit_df, x_thl_df, x_wnt_df, x_pos_df, x_neg_df, x_unc_df, x_bag_df], axis=1, sort=False) 

sel_x_train, sel_x_test, sel_y_train, sel_y_test, sel_z_train, sel_z_test = train_test_split(
    x_sel_df, y_relavence_bag_df, y_sentiment_bag_df, shuffle=True, test_size=0.25, random_state=1)
